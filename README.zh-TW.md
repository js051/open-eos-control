...
- 相機式方向行為：App 固定在裝置自然顯示軸，拍攝布局與設定面板外框不換位，只旋轉需要隨方向閱讀的內容。方向感測只在 Android 系統自動旋轉開啟時運作，並以設定監聽、回到前景校正及有限頻率輪詢同步鎖定狀態。HUD 的 `R6 III`、`82%`、`2,418` 等短標示會在固定方形控制槽內轉向，無障礙描述與可讀狀態面板仍保留完整標籤及數值；取景提示、完整能力警告、長錯誤、設定與動作選單在 90／270 度時會於尺寸受控的可讀面板內交換量測軸，保留完整文字且不移動快門、曝光列或工具列。
- REC 開始/停止
