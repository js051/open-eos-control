...
- 相機式方向行為：拍攝布局與設定面板外框固定不換位，只旋轉需要隨方向閱讀的內容。方向感測只在 Android 系統自動旋轉開啟時運作，並以設定監聽、回到前景校正及有限頻率輪詢同步鎖定狀態。HUD 以 `R6 III`、`2.4K` 等相機式短標示穩定排版，無障礙描述仍保留完整機型與精確張數；較長說明與完整設定內容會先交換量測寬高，再直接定位到目標方向，不移動外層布局。
- REC 開始/停止
