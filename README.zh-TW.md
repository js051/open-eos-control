...
- 相機式方向行為：App 固定在裝置自然顯示軸，拍攝布局與設定面板外框不換位，只旋轉需要隨方向閱讀的內容。前景方向感測會在每次姿態樣本重新讀取 Android 系統「自動旋轉」設定，並以設定監聽、回到前景校正及有限頻率輪詢輔助同步；即使快捷設定通知漏送，也不會保留舊旋轉角度，且 App 沒有可略過系統旋轉鎖的覆寫模式。HUD 短標示與曝光值會在固定方形控制槽內轉向並縮放到可用範圍，無障礙描述與可讀狀態面板仍保留完整相機、電量、儲存與設定值；90／270 度時，完整取景提示使用受限三行 HUD，捲動設定則使用固定底部面板內最大的正方形視窗，完整能力警告、長錯誤、對話框與動作選單也各自在有界面板內顯示，不移動快門、曝光列或工具列。
- REC 開始/停止
