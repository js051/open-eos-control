...
目前的開發預覽版為 [v0.2.0](docs/releases/v0.2.0.md)，用途是測試與收集貢獻者回饋，不建議用於正式拍攝流程。
...
- Android USB/PTP 權限與介面診斷、實際 PTP session、相機身分、儲存卡、媒體瀏覽／縮圖／下載／刪除、依能力開放且具有精確回讀的標準 `SendObjectInfo`／`SendObject` 媒體上傳、由描述符嚴格控管的標準 MTP 媒體評分、相機有公告時的標準拍照／屬性控制，以及依能力開放的 Canon `GetEvent` 機身操作與相機時鐘同步。只有手機與記憶卡兩條路徑都可執行時才顯示拍攝儲存位置選擇；選擇手機會把 Canon 主機 RAM JPEG／RAW 原子傳輸至 App 私有 Media。另包含 Canon EOS 遠端快門、半按、保證取消的原生 AF-ON、拍攝模式、ISO／Tv／Av／白平衡、曝光補償、色溫、白平衡偏移、色彩空間、畫面比例、電動變焦速度、自動關閉電源、自動亮度優化、高 ISO 感光度消除雜訊、AEB、需相符事件回讀的錄影開始／停止、具精確回讀的擁有者／作者／著作權／相機暱稱、自動對焦操作／方式、連續自動對焦、驅動、測光、相片風格、各卡槽 RAW／cRAW／JPEG 畫質、短片伺服自動對焦、焦點移動與 JPEG Live View
- Desktop Bridge 掃描、Bearer 驗證、多相機選擇、session、動態能力／設定、拍攝、AF-ON、Live View、焦點移動，以及依能力提供且可顯示進度／取消的媒體縮圖／上傳／下載／刪除。直接 CCAPI 只有在相機公告內容 `PUT` 時才提供檔案保護、封存狀態、0–5 評分與 0／90／180／270 度顯示旋轉，且每次寫入後都會透過 `kind=info` 回讀驗證；不會宣稱沒有依據的上傳端點。libgphoto2 只有在 runtime 回報 File Upload 且儲存空間可寫時才開放上傳，完成後必須重新列出相機媒體並核對檔名與大小。其路徑也包含經 runtime 探測的 `gphoto2 --wait-event` 機身／媒體同步、R6 Mark III 自動對焦啟動／取消 action、各卡槽影像畫質、白平衡偏移、畫面比例、電動變焦速度、安全的自動關機與自動亮度優化選項、只在可寫 TEXT 路徑出現並強制同路徑精確回讀的擁有者／作者／著作權／暱稱，以及可選擇寫入記憶卡或將主機 RAM 拍攝原子轉存到 Bridge 媒體庫的儲存位置
...
- Android、iOS 與 PC 上依能力開放的 Canon CCAPI Live View，AUTO 依序嘗試 RTP H.264、持續 multipart JPEG、JPEG 輪詢，並限制 1-30 FPS 顯示／輸出幀率。JPEG 與 multipart 必須在同一 API 版本公告一般 `POST /shooting/liveview` 啟動，以及明確的總停止策略：相符的 `DELETE`，或相機只公告 POST 時使用 Canon 文件中的 POST-off payload。Multipart 另要求 `GET`／`DELETE /shooting/liveview/multipart`，背景持續排空資料但只保留最新一張有界完整 JPEG；multipart DELETE 被相機拒絕時仍必定執行總停止。JPEG 第一張遇到 HTTP 503 `Mode not supported` 才條件式降到 `small`，不會依機型永久猜測尺寸。RTP 使用可達路由的 UDP、RFC 3550／RFC 6184、手機原生顯示或 PC PyAV 解碼；Android 另要求五秒內收到具 SPS/PPS 的可解碼關鍵畫面，失敗會先完整清理再讓 AUTO 降級，診斷則保留封包、access unit、keyframe 與 ready 證據。各來源只會在完整能力條件成立時顯示
...
- Android、iOS 與 PC 依能力提供 Canon CCAPI 自動關機時間與立即讓相機休眠。定時選項只接受同一 API 版本成對 GET／PUT 回傳的文件值 `30`／`60`／`120`／`180`／`300`／`600`／`disable`，寫入前會重新讀取相機 ability。特殊值 `immediately` 不會混入一般選單；只有相機明確公告時才顯示為需再次確認的獨立動作，並在相機以 HTTP 202 接受後停止本機 Live View 工作及中斷連線。libgphoto2 與 Android USB 保留已有依據的定時設定，但不宣稱尚未證實的立即休眠命令
- Android、iOS 與 PC 依能力提供需再次確認的 Canon CCAPI 感光元件清潔。只有相機公告 `POST /functions/sensorcleaning` 時才顯示，送出 Canon 要求的嚴格布林關機選項並要求 HTTP 200；一般清潔完成後恢復 Live View，選擇清潔後關機則中斷連線。libgphoto2 與 Android USB 不宣稱沒有可靠公開依據的等效命令
...
- 固定於底部的拍照／錄影模式選擇器會在快門旁持續顯示目前操作情境。相機有公告時會優先使用同一 CCAPI 版本的 `GET`／`POST /shooting/control/moviemode`；否則只在 shooting mode 有相符可寫值時同步機身，兩者都沒有時才維持 App 操作情境。Movie Mode 送出 `{"action":"on|off"}`，與 `recbutton` 開始／停止錄影是不同命令。模式文字與短選取底線會在固定、避開實體邊緣的 48dp 觸控槽內一起旋轉，因此手機側放時不會把整塊選取背景變成遭裁切的直條。模式與曝光寫入期間都會鎖定，完成後再對齊相機確認值，因此遭拒或重疊的請求不會看起來像已套用。
...
USB CLI adapter 現在使用持續的 `gphoto2 --capture-movie --stdout` MJPEG 串流，提供 1-30 FPS 輸出上限；若持續命令失敗，會自動降級成最高 5 FPS 的有界逐幀 `--capture-preview`。相機控制命令執行前會關閉 movie process，下一次取幀時再自動恢復；診斷會保留實際 transport 與 fallback 原因。請求 FPS 不代表相機與 USB 一定能維持該速率。CCAPI engine 同樣提供 1-30 FPS，介面初始預設 15 FPS；HTTP 400 時會移除 `liveviewsize` 重試，JPEG 第一張遇到 HTTP 503 `Mode not supported` 時才降到 `small`，並回報實際尺寸與觀測 FPS。若 discovery 公告 Canon RTP endpoint，且 PyAV 與本機可達 IPv4 都可用，AUTO 依序使用 RTP、multipart、JPEG；每個失敗來源都會先清理，明確選 RTP 則顯示真實錯誤。FPS 在 RTP 下限制解碼後 JPEG 輸出，因 Canon start body 沒有編碼器 FPS 參數。PC 音訊只會在 SDP 公告 in-band `MP4A-LATM/48000` 時啟用；`cpresent=0`、不支援的 codec／rate、bind 或解碼失敗都會誠實顯示且不影響影像。同源 discovery、設定、拍照、快門釋放、錄影、兩種來源下以詳細 metadata 換算的 Tap AF／點選白平衡、有界 JPEG／RTP 影像與音訊解碼、媒體、驗證與能力閘門都有自動測試。2026-08-11 的 R6 Mark III 相機 Wi-Fi 驗證中，RTP 已公告但啟動被 HTTP 503 拒絕；整合後 PC engine 隨即切到 multipart、讀出完整 JPEG 並以 POST-off 停止。之後明確 JPEG 測試也成功取得 medium 完整影格。Android／iOS RTP 音訊／影像及 USB throughput 真機驗證仍待完成。
