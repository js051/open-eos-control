...
目前的開發預覽版為 [v0.8.0](docs/releases/v0.8.0.md)，用途是測試與收集貢獻者回饋，不建議用於正式拍攝流程。

本版新增 Android 直連 CCAPI 的按住 AF-ON，包含放開／取消清理與停止失敗後只送 stop 的重試。PC 與 iOS 只對齊版本，Android USB／Bridge 也尚未加入按住 AF；實機合焦行為與機身半按容易失焦的原因仍待確認，不會自動更改機身 AF 模式。
