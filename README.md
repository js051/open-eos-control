...
- Camera-style orientation that keeps capture geometry fixed and makes every sensor sample honor Android's system auto-rotate setting. Compact controls rotate in stable slots, while longer copy is remeasured against the full available viewport. Settings follow the user's physical bottom edge: a bottom sheet in portrait, a full-length side sheet when held sideways, and a top-anchored sheet in natural coordinates when the phone is upside down.
- REC start/stop
