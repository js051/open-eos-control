...
- Camera-style orientation that keeps capture geometry fixed, gates displayed control rotation on the Android system auto-rotate preference while keeping foreground posture tracking ready, and remeasures rotating text/icons against the available viewport when the phone is held sideways or upside down; natural and non-natural settings share one stable modal surface, switching between a bounded bottom panel and a consistently rotated full-screen panel without losing the open setting
- REC start/stop
