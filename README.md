...
The current development preview is [v0.1.7](docs/releases/v0.1.7.md). It is intended for testing and contributor feedback, not production camera workflows.
...
- Capability-gated Canon CCAPI RTP H.264 Live View on Android, iOS, and PC, with routed UDP reception, RFC 3550/RFC 6184 packet handling, native mobile display or PyAV decode on PC, an Auto/RTP/JPEG source selector, and a 1-30 FPS display/output cap. Android and PC additionally decode advertised in-band RFC 6416 `MP4A-LATM/48000` audio behind default-muted controls; Android uses Media3 LATM extraction, `MediaCodec`, and `AudioTrack`, while PC exposes bounded PCM through WebAudio. RTP appears only when the camera advertises both endpoints and a reachable local IPv4 address plus a working video decoder exist
