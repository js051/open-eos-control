...
The current development preview is [v0.8.0](docs/releases/v0.8.0.md). It is intended for testing and contributor feedback, not production camera workflows.

This preview adds press-and-hold AF-ON to Android direct CCAPI, with release/cancellation cleanup and stop-only recovery after a failed stop. PC and iOS are version-aligned only; held AF is not added to those clients or Android USB/Bridge. Physical AF behavior and the reported camera-body half-press defocus remain unconfirmed; no camera AF-mode settings are changed.
...
- Android USB/PTP permission, interface diagnostics, real PTP sessions, identity, storage, media listing/thumbnail/download/deletion, capability-gated standard `SendObjectInfo`/`SendObject` media upload with exact readback, descriptor-gated standard MTP media rating, advertised standard still capture/property control, capability-gated Canon `GetEvent` body-control and camera-clock synchronization, a Phone/Memory card capture selector with Canon host-RAM JPEG/RAW transfer into app-private Media, and Canon EOS remote release, half-press, native AF-ON with guaranteed cancel, shooting mode, ISO/Tv/Av/WB, exposure compensation, color temperature, white-balance shifts, color space, aspect ratio, power-zoom speed, Auto Power Off, Auto Lighting Optimizer, High ISO noise reduction, AEB, event-verified movie start/stop, bounded Owner/Artist/Copyright/Nickname metadata with exact readback, AF operation/method, Continuous AF, drive, metering, Picture Style, per-card RAW/cRAW/JPEG image quality, Movie Servo AF, focus drive, and JPEG Live View
- Desktop Bridge discovery, Bearer authentication, multi-camera selection, sessions, dynamic capabilities/settings, capture, AF-ON, Live View, focus drive, and ability-gated media thumbnails/upload/download/deletion with transfer progress and cancellation. Direct CCAPI additionally exposes file protection, archive state, 0-5 rating and 0/90/180/270 display rotation only when the camera advertises contents `PUT`, and verifies every change through `kind=info` readback; it does not claim an unverified upload endpoint. The libgphoto2 path enables upload only from runtime File Upload and writable-storage evidence, then verifies the fresh camera listing. It also includes runtime-probed `gphoto2 --wait-event` body/media synchronization, R6 Mark III autofocus drive/cancel actions, per-card image quality, WB shifts, aspect ratio, power-zoom speed, safe Auto Power Off and Auto Lighting Optimizer choices, and a selectable Capture Target that either shoots to card or atomically transfers host-RAM captures into the Bridge media library
...
- Capability-gated Canon CCAPI Live View on Android, iOS, and PC with an `AUTO -> RTP H.264 -> persistent multipart JPEG -> JPEG polling` preference order and a 1-30 FPS display/output cap. JPEG and multipart require a same-version regular `POST /shooting/liveview` start plus an explicit general stop strategy: matching `DELETE`, or Canon's documented POST-off payload when the camera advertises only POST. Multipart additionally requires `GET`/`DELETE /shooting/liveview/multipart`, continuously drains camera data while keeping only the newest bounded complete JPEG, treats a rejected multipart DELETE as best-effort while still sending the general stop, and falls back only in AUTO. A first-frame HTTP 503 `Mode not supported` conditionally retries JPEG at `small`; it does not permanently infer a size from the model. RTP uses routed UDP, RFC 3550/RFC 6184 handling and native mobile display or PyAV decode on PC. Android additionally requires a decodable SPS/PPS-backed key frame within five seconds, cleans up before AUTO fallback, and reports video packet/access-unit/keyframe readiness. Sources appear only when their complete advertised lifecycle and local decoder/route requirements are available
- Dynamic Photo-mode Live View magnification on Android, iOS, and PC. Direct CCAPI requires same-version `GET`/`PUT /shooting/settings/lvzoom`, accepts only Canon's exact string `1`/`5`/`10` ability, sends a string value, and confirms every write with GET readback. Android USB/PTP and libgphoto2 remain limited to their independently verified 1x/5x controls; no backend invents 10x support
...
- Camera media browsing defaults to a bounded **Recent** scope across Android, iOS, and PC: clients request 61 newest items, display 60, and disclose when more remain. **Full card** performs the complete paged traversal without fixed item, page-count, or directory-depth cutoffs. Traversal validates same-origin resources, fairly samples sibling photo/video containers, detects container cycles, rejects malformed page counts and remains cancellable on mobile. A limited direct-CCAPI request stops leaf pagination early; libgphoto2 still completes its single recursive CLI listing before returning a bounded response. Mobile retains at most 96 thumbnails in an LRU cache; PC renders 72 items per page and retains at most 96 revocable thumbnail URLs, so a large card does not turn every result into a simultaneous UI node or decoded image. Full-screen image/RAW display previews and Android, iOS and PC video playback use cancellable byte-range streaming instead of whole-file memory buffering. PC browser, Android and iOS Desktop Bridge playback use one random, 15-minute capability URL per open video so repeated seeks share the same staged file; the URL is scoped to one session and media item, same-origin validated by both native clients, excluded from diagnostics, revoked after the last active stream closes, and invalidated when the session ends. Capability-gated Android USB/PTP and Desktop Bridge JPEG/PNG previews remain bounded to 32 MiB. Downloads and picker uploads stream with progress/cancel; deletion requires confirmation; Canon CCAPI protection, archive state, rating and display rotation require verified readback. Android direct USB upload requires standard PTP `SendObjectInfo`/`SendObject`, writable card storage, a camera-advertised matching format, exact bytes and ObjectInfo readback. Desktop Bridge upload requires runtime libgphoto2 File Upload evidence and a verified fresh name/size match; Android, iOS and PC hide it otherwise, and direct CCAPI never receives a guessed upload request. Android direct USB supports standard PTP card-file protection only when the camera advertises `SetObjectProtection`, with exact ObjectInfo readback. It also supports standard MTP `Rating (0xDC8A)` only after all four object-property operations are advertised, the exact object format reports a writable `UINT16` descriptor that permits `0/20/40/60/80/100`, and a real card object value is read successfully. Five UI stars map to those six wire values; non-integral values remain unknown, and every write requires exact same-handle readback. Unknown states and app-private Phone captures expose no unsupported action. USB archive/display rotation and libgphoto2 media metadata writes remain unavailable; the separately verified camera Owner/Artist/Copyright/Nickname settings are not media-item edits.
- Read-only media details remain available even when metadata writes are unsupported. Android USB/PTP and Desktop Bridge preserve known MIME type, exact size and original pixel dimensions without guessing missing values.
- A single camera item or Android media multi-selection can be handed to a compatible Serein receiver. Open EOS Control fully stages and hashes each original, grants temporary read-only `content://` access, and accepts only an exact-session verified receipt without deleting or modifying camera media.
...
The USB CLI adapter uses persistent `gphoto2 --capture-movie --stdout` MJPEG, accepts a 1-30 FPS output cap, and automatically falls back to bounded per-frame `--capture-preview` at up to 5 FPS when the persistent command fails. Camera-control commands close the movie process before accessing the camera and the next frame request starts it again; diagnostics preserve the effective transport and fallback reason. Requested FPS is not a promise that the camera and USB link will sustain that rate. The CCAPI engine also advertises 1-30 FPS, defaults the UI to 15 FPS, retries without `liveviewsize` after HTTP 400, conditionally retries `small` only when the first JPEG frame returns HTTP 503 `Mode not supported`, and reports the effective size plus observed FPS. When discovery advertises Canon's RTP endpoints, PyAV and a routed local IPv4 receiver are available, `AUTO` prefers RTP, then multipart, then JPEG; every failed source is cleaned before fallback, while explicit RTP reports the real error. The FPS value caps decoded JPEG output because Canon's RTP start body has no encoder-FPS field. Android, iOS, and PC RTP audio are exposed only for advertised in-band `MP4A-LATM/48000`; explicit `cpresent=0`, unsupported codecs/rates, bind failures and decode errors are reported without claiming playback or stopping video. Mobile audio is independently received and parsed even while muted, starts decoding/playback only after an explicit user toggle, and mutes again when the app enters the background. Same-origin discovery, settings, capture, guaranteed shutter release, recording, detail-metadata-backed Tap AF/Click WB under both sources, bounded JPEG/RTP video/audio decoding, media traversal/download/deletion, auth handling, and capability gates are automated-test covered. The browser workflow also supports Auto/RTP/JPEG selection, default-muted camera audio, English/Traditional Chinese, and desktop/narrow layouts. A 2026-08-11 R6 Mark III camera-Wi-Fi validation advertised RTP but rejected its start with HTTP 503; the integrated PC engine then selected multipart, returned a complete JPEG, and stopped with POST-off. Explicit JPEG also returned a complete medium frame in a subsequent run. Physical Android/iOS RTP audio/video and USB throughput validation remain required.
...
- Revalidate the R6 Mark III's now-advertised RTP H.264 path with Android's bounded first-video check and packet/access-unit/keyframe diagnostics; retain another fully advertised Live View source when RTP does not become ready.
