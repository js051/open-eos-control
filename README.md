...
- Camera-style orientation that keeps capture geometry fixed, enables posture tracking only while Android system auto-rotate is enabled, and remeasures rotating text/icons against the available viewport when the phone is held sideways or upside down. Settings keep one fixed bottom-panel geometry; its remeasured content becomes a readable side workspace when the phone is held sideways without moving or recreating the panel.
- REC start/stop
