...
- Camera-style orientation that locks the app to the device's natural display axis, keeps capture and settings geometry fixed, and rotates only direction-aware content. The Activity snapshots Android's system auto-rotate policy before starting the foreground sensor listener, observes the public setting, and reconciles real Quick Settings changes across focus transitions; sensor samples consume that cached policy instead of mistaking a platform orientation synchronization value for a user action. Compact HUD labels and exposure values rotate and shrink to fit inside fixed square control slots, while accessibility descriptions and a readable status panel retain the full camera, battery, storage, and setting values. At 90/270 degrees, complete viewfinder guidance uses a bounded multi-line HUD and scrollable settings remeasure against swapped axes across the full fixed panel; full capability warnings, long errors, dialogs, and the action menu use their own bounded readable panels without moving the shutter, exposure rail, or toolbar.
- REC start/stop
