...
The current development preview is [v0.1.6](docs/releases/v0.1.6.md). It is intended for testing and contributor feedback, not production camera workflows.
...
- Camera-style orientation that locks the app to the device's natural display axis, keeps capture and settings geometry fixed, and rotates only direction-aware content. The foreground sensor listener re-reads Android's live system auto-rotate setting for every posture sample, while setting observation, focus reconciliation, and a bounded poll provide additional synchronization; a missed Quick Settings callback therefore cannot leave stale rotated controls, and no app-level override can bypass the system lock. Compact HUD labels and exposure values rotate and shrink to fit inside fixed square control slots, while accessibility descriptions and a readable status panel retain the full camera, battery, storage, and setting values. At 90/270 degrees, complete viewfinder guidance uses a bounded three-line HUD and scrollable settings use the largest square viewport inside the fixed bottom panel; full capability warnings, long errors, dialogs, and the action menu use their own bounded readable panels without moving the shutter, exposure rail, or toolbar.
- REC start/stop
