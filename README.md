...
- Camera-style orientation that locks the app to the device's natural display axis, keeps capture and settings geometry fixed, and rotates only direction-aware content. The sensor listener runs only while Android system auto-rotate is enabled, with setting observation, focus reconciliation, and a bounded poll keeping lock changes synchronized; there is no app-level override that can bypass the system rotation lock. Compact HUD labels and exposure values rotate and shrink to fit inside fixed square control slots, while accessibility descriptions and a readable status panel retain the full camera, battery, storage, and setting values. At 90/270 degrees, viewfinder guidance, full capability warnings, long errors, settings, and the action menu swap their measurement axes inside bounded readable panels, preserving complete labels without moving the shutter, exposure rail, or toolbar.
- REC start/stop
