...
- Camera-style orientation that keeps capture geometry fixed, observes the Android system auto-rotate preference, disables and clears orientation sensing while rotation is locked, and remeasures rotating text/icons against the available viewport when the phone is held sideways or upside down; non-natural settings use a consistently rotated full-screen panel instead of a mismatched sheet
- REC start/stop
