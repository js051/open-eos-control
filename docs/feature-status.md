...
| PC direct CCAPI engine | Implemented; device validation required | Manual HTTP(S) origin and memory-only Basic Auth, same-origin full-URL/relative-path advertised-operation discovery and bounded evidence, dynamic settings, capture/release, recording, Tap AF, 1-30 FPS bounded JPEG polling with the R6 Mark III HTTP 400 fallback, same-origin media traversal/download/deletion, secret redaction and deterministic tests pass |
