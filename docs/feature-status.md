...
| PC direct CCAPI engine | Implemented; device validation required | Manual HTTP(S) origin and memory-only Basic Auth, same-origin full-URL/relative-path advertised-operation discovery and bounded evidence, dynamic settings, capture/release, independent AF-ON start/stop, recording, Canon detail-metadata-backed `afframeposition` Tap AF and `clickwb` Click White Balance, advertised `drivefocus` Near/Far steps 1-3, 1-30 FPS bounded JPEG polling with the R6 Mark III HTTP 400 fallback, same-origin media traversal/download/deletion, secret redaction and deterministic tests pass |
| PC control UI | Implemented; device validation required | Built into the bridge root URL; USB/CCAPI mode selection, capability-gated controls, authenticated binary Live View/downloads/thumbnails, object-URL cleanup, confirmation-gated deletion, memory-only secrets, capability evidence in redacted diagnostics, English/Traditional Chinese, desktop/narrow browser interaction and overflow checks pass |
