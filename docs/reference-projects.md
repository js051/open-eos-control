...
- Android Canon EOS USB path: capability-gated remote/event setup, ISO/Tv/Av/WB, exposure compensation, color temperature, signed WB shifts, color space, High ISO noise reduction and AEB event state/writes, still capture with balanced release and event confirmation, timed half-press, Card/None movie control, Near/Far focus drive, EVF start/stop, and in-memory JPEG Live View parsing.
...
- Canon EOS USB Touch AF and remaining vendor settings beyond the implemented exposure, color, image-quality, AF, drive, metering and movie mappings.
- CCAPI RTP live view.
