...
- EOS R6 Mark III libgphoto2 evidence: upstream currently includes a generated R6 Mark III configuration snapshot with Canon EOS capture abilities, storage, battery, exposure, remote release, movie target, and manual focus-drive values. It is concrete adapter evidence, but it does not replace this project's physical-device acceptance record. Reference: [upstream R6 Mark III snapshot](https://github.com/gphoto/libgphoto2/blob/master/camlibs/ptp2/cameras/canon-eos-r6-markIII.txt), [gphoto2 CLI manual](https://www.gphoto.org/doc/manual/ref-gphoto2-cli.html).
