...
- CCAPI network backend shape, same-origin canonical `url` and relative `path` discovery parsing, simulator contract, dynamic settings, recording, tap focus, advertised `drivefocus` Near/Far control, JPEG live view polling, still/manual shutter control, paged media browsing, streaming media download, and deletion gated by an advertised `DELETE` operation.
...
- R6 Mark III still/manual shutter, recording, tap focus, CCAPI manual focus drive, media browsing, PTP/libgphoto2 media thumbnails, media download, and media deletion.
