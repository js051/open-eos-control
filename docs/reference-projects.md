...
- Canon EOS PTP behavior reference: pinned libgphoto2 source defines the remote/event operations, balanced release sequence, `DriveLens` values, EVF properties, `GetViewFinderData` request and block format mirrored by the Android backend. References: [operation/property definitions](https://github.com/gphoto/libgphoto2/blob/ce6c5f7c7fdde404e9897f618df6168c01df70f5/camlibs/ptp2/ptp.h), [capture and Live View behavior](https://github.com/gphoto/libgphoto2/blob/ce6c5f7c7fdde404e9897f618df6168c01df70f5/camlibs/ptp2/library.c), [focus and remote controls](https://github.com/gphoto/libgphoto2/blob/ce6c5f7c7fdde404e9897f618df6168c01df70f5/camlibs/ptp2/config.c).
...
- Android Canon EOS USB path: capability-gated remote/event setup, still capture with balanced release and event confirmation, timed half-press, Near/Far focus drive, EVF start/stop, and in-memory JPEG Live View parsing.
- Desktop Bridge service, libgphoto2 CLI adapter, and Android bridge client: authenticated sessions, dynamic settings/capabilities, capture, focus, JPEG preview, media streaming and deterministic contract tests.
...
- Android USB/PTP diagnostics, session, DeviceInfo, standard properties, storage/media, download, advertised standard capture, and the implemented Canon remote/focus/Live View paths on a physical Android device with the camera connected over OTG/USB-C.
- Desktop Bridge and Android bridge client with a physical R6 Mark III.

Planned only, not product-valid yet:

- Canon EOS USB movie control, Touch AF, and vendor exposure/settings not covered by the implemented mappings.
- CCAPI RTP live view.
