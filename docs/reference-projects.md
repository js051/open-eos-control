...
- CCAPI network backend shape, same-origin canonical `url` and relative `path` discovery parsing, simulator contract, dynamic settings, recording, tap focus, JPEG live view polling, still/manual shutter control, paged media browsing, streaming media download, and deletion gated by an advertised `DELETE` operation.
...
- R6 Mark III still/manual shutter, recording, tap focus, media browsing, PTP/libgphoto2 media thumbnails, media download, and media deletion.
