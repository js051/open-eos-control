...
9. Canon capture sends half/full press and balanced release operations, then waits for a captured-object event instead of treating command acceptance as a completed exposure. Manual half-press and Near/Far focus-drive commands use the same prepared session.
10. Canon Live View writes EVF mode/output, requests `GetViewFinderData`, retries documented busy/not-ready responses, validates each response block, and returns only JPEG block types 1 or 11 as in-memory frames.
11. Disconnect stops EVF output, restores Canon remote/event state on a best-effort basis, sends `CloseSession`, and always releases the USB interface and device connection.
...
- Canon Touch AF, movie control, and vendor exposure/settings remain unavailable until their coordinate, state, type, and value semantics are adequately proven.
...
The pinned open-source implementation provides reproducible operation codes, packet shapes, release ordering, focus values and Live View parsing behavior. It is corroborating implementation evidence, not a substitute for an authoritative Canon specification or a recorded physical R6 Mark III validation.
