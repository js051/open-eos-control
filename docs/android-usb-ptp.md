...
12. Canon capture first checks `CaptureDestination (0xD11C)`. For host RAM (`4`), the backend requires app-private capture storage plus advertised standard `GetPartialObject (0x101B)` and Canon `TransferComplete (0x9117)`. When remaining host shots are below 100 it first announces PC capacity through advertised `PCHDDCapacity (0x911A)`. Each `RequestObjectTransfer` event is parsed for handle, format, 32-bit size and filename, downloaded in bounded 1 MiB chunks, atomically finalized, and acknowledged only after the complete file is durable. RAW+JPEG requests are all drained before success. If that path is unavailable, the backend restores a camera-advertised non-host memory-card value before shutter; when neither path exists it fails before shutter. It then sends half/full press and balanced release operations and waits for a card object or completed host transfer instead of treating command acceptance as a completed exposure. Independent AF-ON prefers the advertised no-argument `DoAf (0x9154)` plus `AfCancel (0x9160)` pair and guarantees cancel even after failure or coroutine cancellation; a balanced half-press is retained only as a fallback. Manual half-press and Near/Far focus-drive commands use the same prepared session.
...
- `CaptureDestination` is diagnostic-only and never offered as a user setting. Android follows the camera's current destination: completed host-RAM captures are kept in app-private media and appear in Media for thumbnailing when Android can decode them, Save As, or deletion. A failed partial transfer removes its temporary file and is not acknowledged as complete.
...
- Record whether the complete Canon remote/event operation set is advertised. With host RAM selected for `0xD11C`, capture JPEG, RAW and RAW+JPEG; verify every file appears in Media with the expected byte size, can be saved, and the next exposure is accepted. With host transfer opcodes absent, verify a camera-advertised card destination is written before remote release instead.
