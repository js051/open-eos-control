...
10. Canon exposure compensation, color temperature, signed white-balance shifts, color space, aspect ratio, power-zoom speed, High ISO noise reduction, AEB, AF operation, Continuous AF, AF method, drive, metering, Picture Style and Movie Servo AF use their pinned libgphoto2 data widths and value tables. The backend exposes only choices present in each camera-provided available-value event.
...
- Aspect ratio (`0xD194`, UINT32) and power-zoom speed (`0xD149`, UINT32) use the pinned Canon tables and the same camera-advertised list rule. A one-value list remains visible in diagnostics but is omitted from the normal control sheet because it offers no actionable choice.
