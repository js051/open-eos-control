...
15. Standard `GetObject (0x1009)` also backs display preview only when object metadata identifies JPEG or PNG and advertises a positive size no larger than 32 MiB. The receiver uses a bounded output stream and validates a complete image signature. Host-captured files use the same per-item gate and bounded read. RAW, HEIF and video remain downloadable without a preview action.
16. Disconnect stops EVF output, restores Canon remote/event state on a best-effort basis, sends `CloseSession`, and always releases the USB interface and device connection.
