...
10. Canon exposure compensation, color temperature, signed white-balance shifts, color space, High ISO noise reduction, AEB, AF operation, Continuous AF, AF method, drive, metering, Picture Style and Movie Servo AF use their pinned libgphoto2 data widths and value tables. The backend exposes only choices present in each camera-provided available-value event.
