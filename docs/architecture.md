...
- Android, iOS, and PC CCAPI RTP require advertised `GET /shooting/liveview/rtpsessiondesc` and `POST /shooting/liveview/rtp`, a routed local IPv4 destination, an SDP H.264/90 kHz video description, a bound UDP receiver, and a working decoder. Mobile uses Wi-Fi-bound native render sessions; PC uses PyAV/FFmpeg to decode every H.264 access unit and FPS-cap JPEG output through the stable Bridge frame endpoint. On PC only, an advertised `MP4A-LATM/48000` audio stream with in-band configuration is independently bound, reassembled under RFC 6416, decoded to bounded 48 kHz stereo s16le PCM, and exposed through authenticated long polling; audio failure never invalidates ready video. `AUTO` may prefer RTP only after all video gates pass and falls back to the complete JPEG lifecycle if RTP startup fails. The UI FPS value is a display/output cap; the Canon start contract does not contain an encoder frame-rate parameter.
