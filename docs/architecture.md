...
- Canon CCAPI Live View magnification is a Photo-only `LIVE_VIEW_MAGNIFICATION` capability. It requires a same-version `GET`/`PUT /shooting/settings/lvzoom` pair and a strict unique string ability containing `1` plus at least one of `5` or `10`; the current string must belong to that list. Writes send `{"value":"1|5|10"}` and become successful only after an authoritative GET readback matches. Product UI cycles only the advertised values and never exposes it in Video mode. Android USB/PTP and libgphoto2 advertise only their separately verified 1x/5x values.
...
- Canon CCAPI sound recording level is a separate `SOUND_RECORDING_LEVEL_CONTROL`. It requires an exact same-version `GET` and `PUT /shooting/settings/soundrecording/level`, an exact integer current value, and a bounded integer `min`/`max`/positive `step` ability with 2-256 generated choices containing that value. Android, iOS and PC expose it only as a Video settings slider, re-read the endpoint immediately before every write, and send the selected integer rather than its UI string. HTTP errors caused by Still mode, active recording, busy state or non-manual sound recording remain authoritative failures.
- Canon CCAPI sound recording mode, wind filter and attenuator share `SOUND_RECORDING_CONTROL` while retaining independent settings. Each requires an exact same-version `GET`/`PUT` pair at `/shooting/settings/soundrecording`, `/shooting/settings/soundrecording/windfilter` or `/shooting/settings/soundrecording/attenuator`; the current string and every unique ability value must belong to Canon's documented token set, with at least two choices. Android, iOS and PC re-read all advertised audio settings before a write, send the exact camera token, expose them only in Video settings, and preserve Canon's busy, recording, Still-mode and disabled-audio failures.
- Canon CCAPI focus bracketing is a separate `FOCUS_BRACKETING_CONTROL`. The root `/shooting/settings/focusbracketing` resource must advertise exact same-version GET/PUT and return a unique `enable`/`disable` ability containing the current value before any child is read or exposed. Number of shots and focus increment require exact bounded integer current/min/max/positive-step contracts, while exposure smoothing uses the same exact string contract as the root; every child remains independently capability-gated. Android, iOS and PC force a complete advertised-group re-read before each write, send integers for the two ranges and exact strings for the two switches, expose the group only in Photo settings, and preserve Canon's busy, shooting/recording and Movie-mode failures.
- Canon CCAPI movie recording settings use a separate `MOVIE_SETTINGS_CONTROL`. `moviequality`, `highframerate`, `moviecropping`, and `movieformat` each require an exact same-version GET/PUT pair and a unique bounded string ability containing the current value. High-frame-rate and cropping values are restricted to `enable|disable`; format is restricted to `raw|mp4`; movie-quality tokens remain camera-advertised so newer bodies are not rejected, but count and length are bounded. Android, iOS, and PC re-read all advertised movie resources before each Video-only write and keep readable display summaries separate from the raw token sent to Canon.
- Canon CCAPI camera-function settings `/functions/beep` and `/functions/displayoff` remain independent generic advanced settings. Each requires an exact same-version GET/PUT pair, at least two unique documented strings containing the current value, and a fresh complete group read before an exact string write. Beep permits only `enable|disable|disabletouch`; display timeout permits only `10|20|30|60|120|180`. The controls are available in both Photo and Video contexts when advertised, while malformed, stale, cross-version and Canon busy/recording responses never become optimistic state. Immediate auto-power-off is a separate disconnecting action and is not inferred from `displayoff`.
...
- Canon EDSDK must not be committed or redistributed in this open-source repo. The Bridge exposes only an SDK-neutral, fail-closed Python provider contract; a real licensed provider remains external research until Canon permissions, SDK headers and physical-camera behavior are proven. Different local USB engines are mutually exclusive, and clients reopen the exact engine returned by discovery.
- Camera Live View sources are interchangeable at the contract level: CCAPI JPEG polling, CCAPI persistent multipart JPEG, CCAPI RTP, USB/PTP preview, and bridge streams. AUTO tries only fully advertised sources in the order RTP, multipart, polling and cleans up each failed lifecycle before continuing. PC local UVC/HDMI is a UI-owned supplemental preview source rather than a `CameraControlBackend` capability.
