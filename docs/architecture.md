...
- Android CCAPI RTP requires advertised `GET /shooting/liveview/rtpsessiondesc` and `POST /shooting/liveview/rtp`, a camera-Wi-Fi IPv4 destination, an SDP H.264/90 kHz video description, a bound UDP socket, and a live native decoder session. `AUTO` may prefer RTP only after all gates pass and falls back to the complete JPEG lifecycle if RTP startup fails. The UI FPS value is a render cap; the Canon start contract does not contain an encoder frame-rate parameter.
