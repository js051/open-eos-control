...
- Device-validation queue: still capture, independent AF-ON, shutter half-press, movie recording, tap focus, Click White Balance, manual focus drive, media browser, media thumbnails, media download, and media deletion on EOS R6 Mark III.
...
- Canon still capture and half-press require the full remote/event operation set. A host-RAM `CaptureDestination (0xD11C)` uses advertised `GetPartialObject` in 1 MiB chunks followed by Canon `TransferComplete`; completed files are atomically stored in app-private media and exposed through the existing Media workflow. RAW+JPEG transfer requests are all consumed. If host transfer prerequisites are absent, an advertised non-host card value is restored before shutter; a missing usable destination aborts before shutter. Capture is not reported as success until a card object event or completed host transfer arrives, and press/release commands are balanced on failure paths.
...
- Connection: the PC UI can control a USB camera through libgphoto2 or connect directly to the camera's wireless CCAPI origin. Android and iOS can use the same authenticated bridge for a computer-attached USB camera. Windows uses native gphoto2 when present or a capability-checked WSL 2 command prefix; WSL USB requires usbipd-win plus an explicitly attached camera.
