...
`GET /v1/session/{id}/media` returns the complete list produced by the active engine; clients do not truncate it to 500 items. Direct CCAPI traverses every camera-advertised container and page, while progressive mobile clients may publish completed pages before the request ends. Diagnostics therefore pair the current list size with a load status. Only `COMPLETE` means the count represents a finished traversal; a partial count accompanied by `LOADING`, `CANCELLED`, `FAILED`, or `NOT_LOADED` must not be reported as the card total.
