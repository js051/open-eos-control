...
`GET /v1/session/{id}/media/{itemId}/info` returns the authoritative media item including nullable `protected`, `archived`, `rating`, and `rotationDegrees`. Direct CCAPI enables `MEDIA_PROTECT`, `MEDIA_ARCHIVE`, `MEDIA_RATING`, and `MEDIA_ROTATE` only when discovery advertises contents `PUT`. The corresponding Bridge routes are `PUT .../protection` and `PUT .../archive` with `{"enabled":true|false}`, `PUT .../rating` with `{"value":0..5}`, and `PUT .../rotation` with `{"degrees":0|90|180|270}`. The engine maps these to Canon's documented `protect=enable|disable`, `archive=enable|disable`, `rating=off|1..5`, and `rotate=0|90|180|270` action/value bodies, then performs up to three bounded `kind=info` readbacks. A mismatch is an error and never observed evidence. libgphoto2 returns unsupported for all four routes.
