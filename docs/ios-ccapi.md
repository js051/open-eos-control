...
- Auto/RTP/JPEG source selection when the camera advertises both paths; iOS uses Wi-Fi-only `NWListener` instances, RFC 6184 video depacketization and `AVSampleBufferVideoRenderer`, plus a separately gated default-muted AAC-LATM monitor, while AUTO closes a failed native session before JPEG fallback
...
The macOS CI job builds the app for iOS Simulator, verifies the compiled asset catalog, English/Traditional Chinese resources, launch screen, local-network metadata, and supported orientations, then runs the app unit tests and seven UI workflows on an iPhone Simulator. The retained screenshots cover portrait control, landscape control, landscape Debug, offline media download, confirmation-gated media deletion, offline monitoring settings, Traditional Chinese connection, and Desktop Bridge connection states. A required network workflow also starts the repository's fake camera and drives the production SwiftUI, `CameraAppState`, `OpenEOSCore`, and CCAPI HTTP path through decoded Live View, ISO control, still capture, balanced AF-ON and shutter half-press, coordinate Tap AF, Click White Balance, manual focus drive, mode-gated Bulb start/stop, recording start/stop, decoded media preview, confirmed deletion, and disconnect. Every command is checked against the fake camera's backend state rather than inferred from UI changes. The fake-camera log and XCResult bundle are retained on failure. Physical iPhone and R6 Mark III validation is still required.
