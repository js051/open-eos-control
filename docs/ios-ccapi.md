...
- Same-origin, camera-paginated media traversal without a fixed item, page, or directory-depth ceiling. Direct CCAPI publishes every completed page to the app while traversal continues and checks task cancellation between requests. Canon CCAPI Reference 1.3 content representations on the advertised `GET /contents` operation provide `?kind=thumbnail` thumbnails and `?kind=display` previews only for JPEG/CR3 candidates, plus file-backed downloads with Canon main-file query fallbacks, URLSession byte progress, cancellation, and incomplete-directory cleanup. Individual files may still reject a documented representation; text/JSON metadata is rejected even when returned with HTTP 2xx.
...
- Media listing that progressively displays completed direct-CCAPI pages and allows a manual traversal to be cancelled; file-backed download/share and Bridge upload with real byte progress and accessible cancel actions, a lazy details sheet for capability-gated protection/archive/rating/rotation, confirmation deletion, redacted diagnostic report, and no fake direct-CCAPI or iOS USB/PTP upload action
