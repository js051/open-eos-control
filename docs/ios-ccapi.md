...
- JPEG Live View only when discovery advertises a complete lifecycle: bounded persistent multipart streaming when same-version `GET`/`DELETE /shooting/liveview/multipart` is present, or bounded endpoint polling with cache busting and fallback. Multipart continuously drains the camera stream while retaining only the newest complete JPEG. Both paths retry without `liveviewsize` after Canon returns HTTP 400 `Invalid parameter`.
...
- Canon media protection, rating and display rotation only when discovery advertises contents `PUT`; writes use exact documented action/value bodies and require matching `kind=info` readback before success
...
- Media listing, file-backed download/share and Bridge upload with real byte progress and accessible cancel actions, a lazy details sheet for capability-gated protection/rating/rotation, confirmation deletion, redacted diagnostic report, and no fake direct-CCAPI or iOS USB/PTP upload action
