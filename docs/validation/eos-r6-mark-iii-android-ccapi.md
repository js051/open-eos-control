...
2. Install a current build, exercise each available still, half-press, movie, Tap AF, Click White Balance, storage, browse and download control, and only then copy a fresh diagnostic report. Retain `capabilitySource`, `protocolVersions`, `advertisedCommandCount`, `advertisedCommands`, `writableSettings`, `observedFeatures`, and `capabilityEvidenceTruncated`. Treat `supported` as advertisement evidence and require the matching `observedFeatures` entry before marking a physical control as passed.
