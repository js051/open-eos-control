...
| Still capture and half-press | Pending | The client did not expose these capabilities in this older report. Current clients now parse Canon's full same-origin discovery `url` entries and follow Canon's exact `No list of APIs` / `topurlfordev` fallback, but this camera still needs a fresh capability report and command result |
