...
- Movie mode switched on and restored without recording. Movie quality, movie format, movie cropping, sound recording and internal-microphone wind filter returned valid ability payloads. The separately advertised `moviequality/framerate`, internal-microphone level and attenuator resources returned HTTP 503 `Mode not supported` in this configuration, so no standalone control is inferred from advertisement alone. The movie-quality tokens still preserve their camera-reported frame-rate component.
