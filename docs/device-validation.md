...
For media-library validation, read `mediaItemCount`, `mediaLoadStatus`, `mediaLibraryScope`, and `mediaLibraryHasMore` on Android/iOS, or the matching `mediaLibrary` fields on PC. `COMPLETE` means the selected scope finished; only `scope=all` plus `COMPLETE` means the count came from a traversal that reached every container and page advertised by the camera. The default recent scope requests 61 and displays 60 so the user can inspect new captures without scanning a large card. `LOADING`, `CANCELLED`, `FAILED`, `NOT_LOADED`, and every recent-scope count are intentionally not card totals. There is no 500-item product limit.

Android and iOS also surface progressive counts in the media screen and can cancel the active camera traversal. The PC Bridge exposes an honest loading or failed summary but does not offer a browser cancel control because its current `/media` request is synchronous and cancelling the browser request would not cancel engine work. A limited direct-CCAPI request stops page traversal early; a limited libgphoto2 request still completes its single recursive CLI listing before returning the bounded response.
